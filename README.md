...
- **全服聊天**：所有地圖共用同一頻道，訊息顯示發話者所在區域；新上線可看到最近 25 則
...
伺服器已內建自動破快取：每次更新檔案後，網頁的 script 網址會自動帶上新的 build 代碼，
玩家一重新整理就會抓到新版。登入畫面右下角會顯示 `build xxxxxxxx`，
可用來確認自己載入的是不是最新版（也可打開 `你的網址/version` 查看伺服器目前版本）。

若仍是舊版，通常是 GitHub 檔案還沒更新或 Render 尚未重新部署完成——
到 Render 的 Deploys 頁確認最新一筆是 `Live` 即可。
