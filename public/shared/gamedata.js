...
(function (root, factory) {
  ...
  /* ---------- 一轉技能（等級 350 一轉後解鎖，快捷鍵 T） ----------
     範圍以「一棟商店的寬度」為單位：SHOP_UNIT = 180px */
  const SHOP_UNIT = 180;
  const REBIRTH_SKILLS = {
    warrior:  { id: 'quake', key: 'T', name: '震地斬', icon: '⚡', cd: 15000, mul: 3.0, type: 'slam',
                radius: SHOP_UNIT * 1.5, knock: SHOP_UNIT * 0.5, crit: 0.5,
                desc: '跳起將劍插入地面，對 1.5 個商店大小的範圍造成 300% 傷害，50% 爆擊並將敵人擊退半個商店的距離' },
    archer:   { id: 'slowshot', key: 'T', name: '緩速箭', icon: '❄', cd: 15000, mul: 2.0, type: 'slowshot',
                crit: 0.3, slowMul: 0.4, slowMs: 4000,
                desc: '射出寒冰箭造成 200% 傷害（30% 爆擊、不消耗箭矢），命中目標移動速度降為 0.4 倍持續 4 秒' },
    assassin: { id: 'vanish', key: 'T', name: '隱身突襲', icon: '🌑', cd: 15000, mul: 1, type: 'stealth',
                dur: 3000, speedMul: 1.3, ambush: 3.7,
                desc: '隱身 3 秒（其他玩家看不到你、怪物不會鎖定你），移動速度 +30%；隱身後的第一次普通攻擊造成 370% 傷害（攻擊或放技能會現形）' }
  };
  function skillsOf(cls, reborn) {
    const base = SKILLS[cls] || [];
    return reborn ? base.concat([REBIRTH_SKILLS[cls]]) : base;
  }

  // ---------- 一轉 ----------
  const REBIRTH_LEVEL = 350;      // 到達此等級可一轉
  const REBIRTH_XP_MUL = 1.5;     // 一轉後經驗值 150%

  // ---------- 背包擴充（雜貨商店販售） ----------
  const BAG_BASE = 40;            // 初始格數
  const BAG_STEP = 10;            // 每次 +10 格
  const BAG_MAX = 120;            // 最多擴到 120 格
  const BAG_PRICE_STEP = 100000;  // 第 1 次 10 萬、第 2 次 20 萬…依此類推
  function bagCap(extra) { return BAG_BASE + Math.min(extra || 0, BAG_MAX - BAG_BASE); }
  function bagNextPrice(extra) {
    const bought = Math.round((extra || 0) / BAG_STEP);
    return (bought + 1) * BAG_PRICE_STEP;
  }

  // ---------- PvP 後禁用旅店 ----------
  const INN_PVP_LOCK = 20000;     // 攻擊玩家或被玩家攻擊後 20 秒不能住旅店
  ...
  return {
    ...
    REBIRTH_SKILLS, skillsOf, REBIRTH_LEVEL, REBIRTH_XP_MUL, SHOP_UNIT,
    BAG_BASE, BAG_STEP, BAG_MAX, BAG_PRICE_STEP, bagCap, bagNextPrice, INN_PVP_LOCK,
